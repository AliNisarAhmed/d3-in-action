...
d3.json('https://raw.githubusercontent.com/emeeks/d3_in_action_2/master/data/tweets.json').then(drawHistogram);
...
function drawHistogram(incomingData) {
  ...
  histoChart.domain([0, 5]).thresholds([0, 1, 2, 3, 4, 5]).value(function (d) {
    return d.favorites.length;
  });
  var histoData = histoChart(incomingData.tweets);
  console.log(histoData);
  d3.select('#svgHisto').selectAll('rect').data(histoData).enter().append('rect').attr('x', function (d) {
    return xScale(d.x0);
  }).attr('y', function (d) {
  ...
  }).style('fill', '#FCD88B').on('click', retweets);
...
},{}]},{},["../../.nvm/versions/node/v11.1.0/lib/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","histoLayout.js"], null)
//# sourceMappingURL=/histoLayout.f8dd20df.map